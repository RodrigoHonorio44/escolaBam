      ...
      <div className="print-area max-w-4xl mx-auto bg-white shadow-2xl p-10 md:p-16 border border-slate-200 rounded-sm min-h-[297mm]">
        ...
        <div className="border-b-4 border-slate-900 pb-6 mb-8 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mt-2">{dados.escola || "UNIDADE DE SAÚDE ESCOLAR"}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-400 uppercase">Documento ID</p>
            <p className="text-lg font-black text-blue-600 uppercase italic">#{dados.id?.substring(0,8).toUpperCase()}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="col-span-2 border-b border-slate-100 pb-2">
            ...
            <p className="font-bold text-slate-800 uppercase text-base">{dados.nomePaciente || dados.pacienteNome}</p>
          </div>
        ...
        <div className="space-y-6 mb-8">
          <div>
            ...
            <p className="text-sm text-slate-700 leading-relaxed italic">
              {dados.motivoAtendimento || dados.motivoEncaminhamento || dados.relatoCurto || "Paciente admitido para avaliação."}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 mb-8">
          ...
          {statusAtual === 'Finalizado' && (
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100" style={{ WebkitPrintColorAdjust: 'exact' }}>
              ...
              <p className="text-[12px] font-bold text-slate-800 leading-snug">{dados.condutaHospitalar || "Sem diagnóstico registrado."}</p>
        ...
        <div className="mt-20 grid grid-cols-2 gap-16">
          <div className="text-center border-t border-slate-300 pt-4">
            <p className="text-[11px] font-black uppercase text-slate-900 leading-none">
              {dados.finalizadoPor || dados.profissionalNome || "Profissional Responsável"}
            </p>
            <p className="text-[9px] font-bold text-blue-600 uppercase mt-1">
              Registro: {dados.registroFinalizador || dados.profissionalRegistro || "COREN ATIVO"}
            </p>
          </div>
          
          <div className="text-center border-t border-slate-300 pt-4">
            <p className="text-[9px] font-bold text-slate-400 uppercase italic leading-none">Assinatura do Responsável / Ciente</p>
            <p className="text-[10px] font-black text-slate-300 mt-6">__________________________________________</p>
          </div>
        </div>
